...
const ForkMockContext = createContext<ForkMockContextValue | undefined>(
	undefined,
);
...
export const ForkMockProvider = ({
	children,
}: ForkMockProviderProps): React.JSX.Element => {
	...
	const generateRisk = useCallback(
		(_percentage: number) => {
			if (!isDemoAvailable) return;

			const generatedData = generateDemoForkRiskData(
				DisputeBondScenario.LOW_RISK,
			); // Legacy fallback
